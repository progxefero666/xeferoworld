...
import { MetalMaterials } from '../materials/metalmaterials';


/**
 *class SpheresGenerator.generateSphere
 */
export class SpheresGenerator {  

    public static MINISPHERES_RADIUS_MIN: number = 0.005;
    public static MINISPHERES_RADIUS_MAX: number = 0.6;


    public static generateSphere(radius:number,subdiv:number,color:any): THREE.Mesh {       
        //const spCenter: Vector3d = System3d.CC;
        const geometry = new THREE.SphereGeometry(radius,subdiv,subdiv);
        const material:THREE.MeshStandardMaterial = MetalMaterials.getMaterial(color, 0.9, 0.1, 1.0);
    ...
    public static getAleatMiniSphereRadius(): number {
        const range: number = SpheresGenerator.MINISPHERES_RADIUS_MAX - SpheresGenerator.MINISPHERES_RADIUS_MIN;
        return (range) * Math.random() + SpheresGenerator.MINISPHERES_RADIUS_MIN;
    };//end
    ...
    public static getAleatMiniSphere(spCenter:Vector3d,
        ...
        const radius: number = SpheresGenerator.getAleatMiniSphereRadius();
    ...
    public static getAleatMiniSpheres(countObjects: number,
        ...
        for (let objIndex = 0; objIndex < countObjects; objIndex++) {
            const miniSphere = SpheresGenerator.getAleatMiniSphere(spCenter,spRadiusMin,spRadiusMax);
