...
export class GenHeightMapFunctions {
    ...
    public static getAleatoryGradParams(paramsRanges: NumberParameter[]) {
        ...
        const rotation: number = XMath2dUtil
            .getAleatNumberInRange(paramsRanges[4].range.min, paramsRanges[4].range.max);

        const position: Point2d = PlaneUtil.findSafeRandomPosition(radius);
    ...
    public static generateGradient(id: string, params: Keyvalue[]): Gradient {
        ...
        return new Gradient(id, position, radius, scale, rotation, intensity);
    };//end

    public static generateGradients = (paramRanges:NumberParameter[],count:number):Gradient[]  => {          
        const gradients:Gradient[] = [];
        for(let idx=0;idx<count;idx++){
            const elem_id:string = 'new_' + idx.toString();
            const elem_params:Keyvalue[] = GenHeightMapFunctions.getAleatoryGradParams(paramRanges);
            const elem:Gradient = GenHeightMapFunctions.generateGradient(elem_id, elem_params);
            gradients.push(elem);
        }
        return gradients;
    };//end

    public static getGradientIndex(id:string,gradients: Gradient[]): number {
        let index: number = -1;
...
export class GeneratorTerrainFunc {
    ...
    public static genHeightMapTextureData(size:number,gradients:Gradient[]): ImageData {

        const width = size;
        const height = size;

        //alert(gradients.length);
    ...
    public static getTerrainMaterial = (textureData: ImageData): MeshStandardMaterial => {
        const material: MeshStandardMaterial = GeoMaterials.getGeoObjMaterial("#ffffff", 1.0, 0.5, 0.5);
        if (textureData) {
    ...
    public static generateTerrain = (terrainCfg: HeightMapTerrainConfig,
        ...
        if (modeWire) {
            terrain_material = GeoMaterials.getGridMaterial(wireColor, 1.0);
        }
        else {
