...
import { XMath2d } from "src/math2d/xmath2d";
import { Vector2d } from "./math2dtypes";
import { TDimension } from "@/common/types";


/**
 * class XMath2dUtil.getAleatoryNumberInRange
 */
export class XMath2dUtil {
    ...
    public static toRadians(valueDegrees: number): number {
        return valueDegrees * XMath2d.DEG_TO_RAD_FACTOR;
    };//end

    public static toGrades(valueRad: number): number {
        return valueRad * XMath2d.DEG_TO_RAD_FACTOR;
    };//end
    ...
    public static getDistancePoint(points:Vector2d[],distance:number):Vector2d {
        let index: number = 0;
