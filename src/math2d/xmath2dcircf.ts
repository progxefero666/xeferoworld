...
/**
 * class XMath2dCircf.getCfPoints
 */
export class XMath2dCircf{

    public static getCfPoint(center:Vector2d,radius:number,angle_y:number):Point2d{
		var coord_x = center.x + ( radius * Math.cos(angle_y));
		var  coord_y = center.y + ( radius * Math.sin(angle_y));		
		return {x:coord_x,y:coord_y};	
	}//end
    
	public static getCfPoints(center:Vector2d,radius:number,countPoints:number):Point2d[]{
		const cfPoints: Point2d[] = [];
		const angleUnit:number = (Math.PI * 2) / countPoints;
		let angle:number = 0;
		for(let idx=0;idx<countPoints;idx++) {
			cfPoints.push(XMath2dCircf.getCfPoint(center, radius, angle));
    ...
    public static getCfDiameterLine(center:Vector2d,radius:number,angle:number):Line2d{
		...
		const start:Vector2d = XMath2dCircf.getCfPoint(center,radius,angle);
		const end:Vector2d = XMath2dCircf.getCfPoint(center,radius,angleEnd);
	...
	public static getCfArcPiLine(center:Vector2d,radius:number,angle:number):Line2d {
		...
		const pointStart:Vector2d = XMath2dCircf.getCfPoint(center,radius,angle);
		const pointEnd:Vector2d = XMath2dCircf.getCfPoint(center,radius,angleEnd);
	...
	public static getCfArcPiPoints(center:Vector2d,radius:number,angle:number):Point2d[] {
		...
		for(let idx=0;idx<countPoints;idx++) {
			const currPoint:Point2d = XMath2dCircf.getCfPoint(center,radius,currAngle);
